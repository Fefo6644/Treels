...
LRESULT MainWindow::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_KEYDOWN:
		switch (wParam) {
		case VK_UP:
			if (isAutomated == FALSE && keyArrUp == FALSE)
				keyArrUp = TRUE;
			return 0;

		case VK_DOWN:
			if (isAutomated == FALSE && keyArrDown == FALSE)
				keyArrDown = TRUE;
			return 0;
		...
		case 0x52:      // R - random
			if (isAutomated == FALSE) {
				multiplier = (size_t)5000 - (size_t)std::rand() % (size_t)5001;
				OnPaint();
				InvalidateRect(m_hwnd, NULL, FALSE);
			}
			return 0;
...
void MainWindow::CheckArrowKeys() {
	while (TRUE) {
		if (keyArrUp == TRUE)
...
int MainWindow::OnCreate() {
	tKeys = std::thread(&MainWindow::CheckArrowKeys, this);
	tKeys.detach();
